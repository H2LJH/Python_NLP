...
db = client['moviereply']
collection = db.get_collection('movie')

reply_list = []

def mongo_selectAll():
    for one in collection.find({}, {'_id': 0, 'movieNm': 1, 'content': 1, 'score': 1}): # 제목, 내용, 평점
...
# selctword랑 my model 불러서 저기에 저장해놓은거
...
def predict_pos_neg(review):
    ...
    score = float(model.predict(data)) # model.fit는 학습훈련
    global pos_count
    if (score > 0.5):
        pos_count +=1
